...
const normalizePropertyId = (propertyId) => {
  const value = propertyId?.toString?.().trim();
...
export async function unsaveProperty(propertyId) {
  try {
    ...
    const response = await fetch(`/api/saved-properties/${normalizedPropertyId}`, {
      method: 'DELETE',
      headers: {
        'X-User-Email': email,
      },
    ...
    if (!response.ok) {
      throw new Error(data?.message || 'Unable to unsave property.');
    }
...
export async function checkSavedStatus(propertyId) {
  try {
    const userData = JSON.parse(localStorage.getItem('user') || '{}');
    const email = userData.email;
    ...
    const response = await fetch(`/api/saved-properties/${normalizedPropertyId}/check`, {
      headers: {
        'X-User-Email': email,
      },
    });
